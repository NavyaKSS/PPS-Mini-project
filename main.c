...
char catagories[][16]={"Mathematics","Computers","Physics","Chemistry","Language","Miscellaneous"}; 	//global variables
...
void welcome()	//function to display welcome screen
{
	...
	printf(".");
	sleep(1);
	system("cls");		//it is used as a trick to hold the output screen live for a desired period of time by the user
	login();
	mainmenu();
}
char u[30],p[30];
...
void deletebooks()    //function that delete from file fp
{
	...
	if(findbook=='t' )
	{
	    ...
	    if(getch()=='Y')
	    {
		ft=fopen("temp.txt","wb+");   //for deleting temporary files
		...
		fclose(fp);
		remove("Project.txt");
		rename("temp.txt","Project.txt"); 		//read all items from temporary file except we want to delete
		fp=fopen("Project.txt","rb+");    
		if(findbook=='t')
		{
		    gotoxy(10,10);
...
void searchbooks()	    //function that search items from file fp
{
    ...
    gotoxy( 15,20);
    printf("Enter Your Choice");
    fp=fopen("Project.txt","rb+"); 		//open file in read mode
    rewind(fp);   		//sets the file position to the beginning of the file of the given stream
    switch(getch())
    {
	  case '1':
...
void issuerecord()  	//function that displayed the book's information
{
		 system("cls");
		 gotoxy(10,8);
		 printf("The Book has taken by %s",r.stname);
		 gotoxy(10,9);
